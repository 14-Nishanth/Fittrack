...
const foods = [
  ...
  {name:'Moong Dal',serving:'100 g cooked',calories:105,protein:7.0,carbs:19.2,fat:.4,fiber:7.6,sugar:2.0,sodium:2,emoji:'🥣'},
  ...
  {name:'Biryani, chicken',serving:'1 cup',calories:330,protein:18,carbs:42,fat:11,fiber:2,sugar:2,sodium:650,emoji:'🍛'},
...
export default function FoodPage(){
  ...
  return <main className="app"><header className="top"><div className="brand">Fit<span>Track</span></div><nav className="nav"><a className="pill" href="/">Today</a><a className="pill" href="/workout">Workout</a><span className="pill active-pill">Food</span><a className="pill" href="/profile">Profile</a></nav></header>
    <section className="hero"><h1>🍎 Food & Nutrition</h1><p>Indian foods, meals and everyday foods with calories and nutrition.</p></section>
    ...
    <div className="section">Food database · {filtered.length} foods</div>
    <div className="food-list">{filtered.map(f=><article className="card nutrition" key={f.name}>
      <div className="food-image"><span>{f.emoji}</span><img src={'https://source.unsplash.com/640x360/?'+encodeURIComponent(f.name+',food')} alt={f.name} loading="lazy" onError={e=>{e.currentTarget.style.display='none'}}/></div>
      <div className="food-head"><div><strong>{f.name}</strong><small>{f.serving}</small></div><b>{f.calories} kcal</b></div>
      <div className="nutrients"><span>Protein <b>{f.protein}g</b></span><span>Carbs <b>{f.carbs}g</b></span><span>Fat <b>{f.fat}g</b></span><span>Fiber <b>{f.fiber}g</b></span><span>Sugar <b>{f.sugar}g</b></span><span>Sodium <b>{f.sodium}mg</b></span></div>
      <button onClick={()=>setSelected([...selected,f])}>+ Add to {meal}</button>
    </article>)}</div>
    ...
    <p className="note">Nutrition values are representative estimates; recipes, brands and portions can change the numbers.</p>
  </main>
}
